            ...
            <div className="relative" ref={menuRef}>
              ...
              {showProfileMenu && (
                <div className={`absolute right-0 mt-2 w-64 border rounded-xl shadow-2xl py-2 overflow-hidden z-[70] animate-in fade-in slide-in-from-top-2 duration-200 ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                  ...
                  {currentUser?.role === 'admin' && (
                    <button
                      onClick={() => { onViewChange('billing-admin'); setShowProfileMenu(false); }}
                      className={`w-full flex items-center gap-3 px-4 py-2 text-sm font-medium transition-colors ${activeView === 'billing-admin' ? 'text-green-500' : (darkMode ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-700 hover:bg-slate-100')}`}
                    >
                      <Zap size={16} />
                      Billing Admin
                    </button>
